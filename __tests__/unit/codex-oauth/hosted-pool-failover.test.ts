type HostedPoolFailureReason = 'quota_exhausted' | 'failed' | undefined;
...
const actionBundle = jest.requireActual(
...
describe('hosted pool account failover', () => {
  ...
  it('bounds exhausted-pool retries to three lease attempts', async () => {
    const attempts: number[] = [];
